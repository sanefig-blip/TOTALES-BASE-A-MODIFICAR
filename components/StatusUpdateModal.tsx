...
interface StatusUpdateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (truckId: string, newStatus: Status) => void;
  truck: Truck | null;
}

const statusOptions: Status[] = ['Para Servicio', 'Reserva', 'Alternativa', 'A Préstamo', 'Fuera de Servicio'];
...
const StatusUpdateModal: React.FC<StatusUpdateModalProps> = ({ isOpen, onClose, onSave, truck }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedStatus) {
      onSave(truck.id, selectedStatus);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-gray-800/50 px-6 py-4 flex justify-end space-x-3 border-t border-gray-700">
            ...
            <button
              ...
              disabled={!selectedStatus || selectedStatus === truck.status}
            >
              Guardar Cambios
            </button>
          </div>
